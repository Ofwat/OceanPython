...
def read_pc_sheet(excel_sheet, pc_sheet, outcome_performance_type, pc_records, common_data):
    ...
    for i in range(len(pc_data)):
        if "3A" == pc_sheet or "3B" == pc_sheet:
            pc_record = PCRecord(spreadsheet_updated, outcome_performance_type, pc_data.iloc[i, 0], pc_data.iloc[i, 1],
                                 pc_data.iloc[i, 2], pc_data.iloc[i, 3], pc_data.iloc[i, 4], pc_data.iloc[i, 5],
                                 pc_data.iloc[i, 6], pc_data.iloc[i, 7], pc_data.iloc[i, 8],
                                 common_data.company_acronym, common_data.company_name, common_data.year,
                                 submission_status_actual, common_data.excel_user, common_data.excel_file,
                                 common_data.comment)
        elif "3E" == pc_sheet:
            pc_record = PCRecord(spreadsheet_updated, outcome_performance_type, pc_data.iloc[i, 0], pc_data.iloc[i, 1],
                                 pc_data.iloc[i, 2], pc_data.iloc[i, 3], None, pc_data.iloc[i, 5], pc_data.iloc[i, 6],
                                 None, None, common_data.company_acronym, common_data.company_name, common_data.year,
                                 submission_status_actual, common_data.excel_user, common_data.excel_file,
                                 common_data.comment)
        ...
        if "2020-21" == common_data.year and not isCellEmpty(pc_data.iloc[i, 4]):
            if "3A" == pc_sheet or "3B" == pc_sheet:
                pc_record = PCRecord(spreadsheet_updated, outcome_performance_type, pc_data.iloc[i, 0],
                                     pc_data.iloc[i, 1], pc_data.iloc[i, 2], pc_data.iloc[i, 3], None,
                                     pc_data.iloc[i, 4], '-', None, None, common_data.company_acronym,
                                     common_data.company_name, "2019-20", submission_status_past_performance,
