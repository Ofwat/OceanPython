...
df = pd.read_excel(r'../resources/AnnesPR14.xlsx',
                   sheet_name='PC list', usecols=("A:DB,DU:DZ,EC:EJ"), skiprows=[0])
df.columns = ['unique_id', 'company_type', 'company', 'element_name', 'element_acronym', 'outcome', 'pc_ref', 'annex',
              ...
              'underp_payment_deadband_2016_17', 'underp_payment_deadband_2017_18', 'underp_payment_deadband_2018_19',
...
df = df.astype({'unique_id': 'object', 'company_type': 'object', 'company': 'object', 'element_name': 'object',
                ...
                'in_period_odi': 'object', 'vanilla_odi': 'object', 'primary_category': 'object', 'pc_unit': 'object',
                ...
                'financial_odi_2017_18': 'object', 'financial_odi_2018_19': 'object', 'financial_odi_2019_20': 'object',
...
for i in mylist:
    ...
    if i in ['drinking_water_quality_compliance', 'water_quality_contacts', 'supply_interruptions_3_hours',
             'pollution_incidents_cat_3', 'internal_sewer_flooding', 'scheme_specific_factor', 'asset_health', 'nep',
             'aim']:
        df[i] = np.where(df[i].values == 'Yes', 'TRUE', None)
    if i in ['underp_payment1_incentive_rate_gbpm', 'underp_payment2_incentive_rate_gbpm',
        ...
        df['isnumeric_' + i] = (df[i].apply(lambda x: isinstance(x, (int, np.int64)))) | (
            df[i].apply(lambda x: isinstance(x, (float, np.float64))))
        df['isnumeric_' + i].astype('bool')
        df['onlynumeric_' + i] = ''
        df['onlynumeric_' + i] = np.where(df['isnumeric_' + i] == True, df[i], None)
        na_mask = df['onlynumeric_' + i].notnull()
        df.loc[na_mask, 'onlynumeric_' + i] = df.loc[na_mask, 'onlynumeric_' + i].astype(float).round(18)
...
df1.columns = ['company', 'unique_id', 'outcome', 'pc_ref', 'performance_commitment', 'pc_short_description',
               ...
               'common_and_comparable_bespoke_performance_commitment', 'special_cost_factor', 'scheme_specific_factor',
               'asset_health', 'nep', 'aim', 'customers_relative_priority', 'pcl_2020_21', 'pcl_2021_22', 'pcl_2022_23',
               'pcl_2023_24', 'pcl_2024_25', 'financial_odi_2020_21', 'financial_odi_2021_22', 'financial_odi_2022_23',
               ...
               'underp_payment_deadband_2021_22', 'underp_payment_deadband_2022_23', 'underp_payment_deadband_2023_24',
...
df1 = df1.astype({'company': 'object', 'unique_id': 'object', 'outcome': 'object', 'pc_ref': 'object',
                  ...
                  'asset_health': 'object', 'nep': 'object', 'aim': 'object', 'customers_relative_priority': 'object',
...
for j in mylist1:
    df1[j] = df1[j].replace('-', np.nan)
    df1[j] = np.where(df1[j].isnull(), None, df1[j])
    if j in ['scheme_specific_factor', 'asset_health', 'nep', 'aim']:
        df1[j] = np.where(df1[j].values == 'Yes', 'TRUE', None)
    if j in ['underp_payment_incentive_standard_underp_payment1_tier2_where_tiers_apply',
        ...
        df1['isnumeric_' + j] = (df1[j].apply(lambda x: isinstance(x, (int, np.int64)))) | (
            df1[j].apply(lambda x: isinstance(x, (float, np.float64))))
        df1['isnumeric_' + j].astype('bool')
        df1['onlynumeric_' + j] = ''
        df1['onlynumeric_' + j] = np.where(df1['isnumeric_' + j] == True, df1[j], None)
        na_mask = df1['onlynumeric_' + j].notnull()
        df1.loc[na_mask, 'onlynumeric_' + j] = df1.loc[na_mask, 'onlynumeric_' + j].astype(float).round(18)
...
df2.columns = ['unique_id', 'company_type', 'company', 'element_acronym', 'pc_ref', 'performance_commitment',
               'odi_type', 'primary_category', 'pc_unit_description', 'starting_level_pr14_fd_2014_15', 'pcl_2015_16',
               ...
               'submeasure_low_2016_17', 'submeasure_low_2017_18', 'submeasure_low_2018_19', 'submeasure_low_2019_20',
...
df2 = df2.astype({'unique_id': 'object', 'company_type': 'object', 'company': 'object', 'element_acronym': 'object',
                  ...
                  'pcl_2017_18': 'object', 'pcl_2018_19': 'object', 'pcl_2019_20': 'object', 'sub_measure_id': 'object',
...
for k in mylist2:
    df2[k] = df2[k].replace('-', np.nan)
    df2[k] = np.where(df2[k].isnull(), None, df2[k])
    if k in ['actual_performance_level_pcs_submeasures_pcl_met_2015_16',
             ...
             'actual_performance_level_pcs_submeasures_pcl_met_2019_20',
             'actual_performance_level_pcs_submeasures_pcl_met_estimate_2019_20']:
        df2[k] = df2[k].replace(['Yes', 'No'], ['TRUE', 'FALSE'])

# Connect to SQL Server
with open("../configurationFiles/config.yml", "r") as ymlfile:
    cfg = yaml.safe_load(ymlfile)

dbinformation = cfg["mssql"]
print(dbinformation)
conn = pyodbc.connect('Driver='+ dbinformation["Driver"]+';Server='+ dbinformation["Server"]+';Database='+ dbinformation["Database"]+';UID='+ dbinformation["UID"]+';PWD='+ dbinformation["PWD"])
cursor = conn.cursor()
cursor.execute("SELECT * FROM sys.schemas WHERE name = "+ "'" + dbinformation["SchemaName"] + "'")
